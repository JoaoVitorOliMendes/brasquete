...
const GroupMemberList = ({ members, separator = false, admin }: GroupMemberListProps) => {
    ...
    // Fkin mess
    // But i liked it
    const groupedArray = members && members.group_member ? members.group_member.reduce<{ [key: number]: GroupMember[] }>((obj, item) => {
    ...
    const inviteMembers = () => {
        if (members) {
            ...
            Share.share({
                ...
                message: 'Compartilhe esse link aos integrantes do grupo',
                url: link
            })
        } else
    ...
    const removeMemberConfirmModalMemo = useMemo(() => {
        return <ConfirmModal message={`Você tem certeza que deseja remover o usuário ${focusMember?.profiles?.first_name + ' ' + focusMember?.profiles?.last_name} do grupo?`} title='Remover usuário' onConfirm={() => {
